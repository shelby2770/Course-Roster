...
const Content = ({ content }) => {
  //   const { img, title, desc, price, credit } = content;
  return (
    <div className="bg-base-100 shadow-xl p-2 rounded-lg">
      ...
      <div className="py-2 flex flex-row justify-between">
        <div className="flex flex-row items-center gap-2">
          <FaDollarSign />
          <h3 className="text-gray-500 text-sm">Price : {content.price.substring(1)}</h3>
        </div>
        <div className="flex flex-row items-center gap-2">
          <FaBookOpen />
      ...
      <button className="w-full btn rounded-md bg-[#2F80ED] text-white"> Select</button>
    </div>
  );
};
